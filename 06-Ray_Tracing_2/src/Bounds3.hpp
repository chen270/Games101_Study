                                ...
                                const std::array<int, 3>& dirIsNeg) const
{
    ...
    // 与 包围盒 是否相交
    // dirIsNeg: 光线是否反向, 若光线反向则小于0

    // 列出等式
    //ray.origin + t_min * ray.direction = this->pMin;
    //ray.origin + t_max * ray.direction = this->pMax;
    ...
    float t_enter = std::fmax(std::fmax(t_min_x, t_min_y), t_min_z);
    float t_exit = std::fmin(std::fmin(t_max_x, t_max_y), t_max_z);
